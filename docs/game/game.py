...
def new_state(hero, slot=1):
    return {'hero': hero, 'world': {'zone': 'sealed_cave', 'floor': 1, 'day': 1, 'phase': 0, 'slot': slot, 'phase_name': 'Cave Awakening', 'bosses_slain': [], 'unlocked_zones': [], 'flags': {}, 'recruit_pool': None, 'deepest_floor': 1}, 'roster': [], 'captures': [], 'kingdom': None, 'postgod': None}
...
def load_state(data):
    ...
    return {'hero': hero, 'world': world, 'roster': list(data.get('roster', [])), 'captures': list(data.get('captures', [])), 'kingdom': kingdom, 'postgod': postgod}

async def main():
    _seed_from_env()
    mode, data = await title.title_screen()
    if mode == 'quit':
