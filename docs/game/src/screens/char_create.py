...
async def char_create():
    ui.title_banner()
    intro = 'You died on Earth. The World Language whispers across the void, and your soul is cast into the Great Jura Forest of another world - as a monster. Weak. Nameless. Hungry.'
    ...
    ri = await ui.choose('What are you reborn as?', opts)
    rid = race_ids[ri]
    name = ''
    while not name.strip():
        from src.core import webbridge
        name = (await webbridge.ask_text('Name your monster:') or '').strip()[:16]
        if not name.strip():
            print(ui.R + 'A nameless thing cannot exist.' + ui.RESET)
    hero = Player(name, rid)
    ui.voice([f'Confirmed. Individual [{name}] has been reincarnated.', f"Race: {races_data.RACES[rid]['name']}.", f"Unique characteristics granted: {', '.join(_skill_names(races_data.RACES[rid]['intrinsics']))}."])
    ...
    for l in ui.panel(' GREAT SAGE (dormant) ', [f'A voice stirs within you...', f'Something sleeps in your soul: {hint}.'], ui.C):
        print(l)
    return hero
