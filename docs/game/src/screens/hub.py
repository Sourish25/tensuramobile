...
async def status_view(state):
    h = state['hero']
    ui.clear()
    ep = h.compute_ep()
    lines = [f"{h.color}{h.glyph} {h.name}{ui.RESET}  -  {ui.BW}{h.stage_name}{ui.RESET}  (Race: {races_data.RACES[h.race_id]['name']})", f'Level {h.level}   XP {h.xp:.0f}/{h.xp_to_next()}  [{ui.bar(h.xp, h.xp_to_next(), 20, color=ui.XP_C, show_nums=False)}]', f'HP [{ui.bar(h.hp, h.max_hp, 24)}]   MP [{ui.bar(h.mp, h.max_mp, 18, color=ui.MP_C)}]', f'EP {ep:,}   Guild Rank Estimate: {ui.GOLD_C}{rank_from_ep(ep)}{ui.RESET}', '', f"ATK {h.stats['atk'] + h.weapon_atk():>4}  DEF {h.stats['def'] + h.armor_def():>4}  MAG {h.stats['mag']:>4}  SPR {h.stats['spr']:>4}  AGI {h.stats['agi']:>4}", f"Weapon: {(idata.ITEMS[h.gear['weapon']]['name'] if h.gear['weapon'] else '(none)')}    Armor: {(idata.ITEMS[h.gear['armor']]['name'] if h.gear['armor'] else '(none)')}", f"Gold {ui.GOLD_C}{h.gold}{ui.RESET}   Souls {ui.M}{h.souls:,}{ui.RESET}   Devours {h.devour_count}   Kills {h.kills}   MP scar {h.mp_scar:.0f}/{h.stats['mp']}"]
    titles = []
    if h.demon_lord:
...
async def skills_menu(state):
    ...
    while True:
        ...
        for l in ui.panel(f' SKILLS - {len(h.skills)} known ', lines, ui.B):
            print(l)
        c = await ui.choose('Skills:', ['Synthesize (Great Sage)', 'Back'])
        if c == 0:
            await predator_sys.offer_combination_menu(state)
            continue
        return
...
async def postbattle_souls(state, enemies, auto=False):
    ...
    for e in enemies:
        ...
        c = await ui.choose(f"The {e.name}'s remains:", opts)
        if c == 0:
            dev = predator_sys.devour_one(state, e)
            for mat, n in dev['materials'].items():
                h.materials[mat] = h.materials.get(mat, 0) + n
...
async def defeat_flow(state):
    ...
    print()
    for l in ui.text_panel(' DARKNESS ', f'Your body scatters into motes of magicules. Something ancient drags your essence back toward safety. You reform slowly... {lost} gold slipped away.', ui.R):
        print(l)
    print()
    revive_party(state)
    await ui.pause()

async def on_boss_defeated(state, zid, boss_id):
    zone = mdata.ZONES[zid]
    nxt = zone.get('unlock') or []
    if isinstance(nxt, str):
        nxt = [nxt]
    if boss_id == 'tempest_serpent':
        ui.voice(['The guardian of the depths has fallen.', 'Its magicule core saturates your body.', 'Confirmed. The path beyond the cave is now open.'])
        print()
        for l in ui.text_panel(' THE WORLD OPENS ', 'Sunlight pours through the cave mouth for the first time. The Great Jura Forest stretches endlessly before you. Somewhere out there: goblins in need of a leader, beasts to hunt, and a destiny only you can name.', ui.Y):
            print(l)
...
async def explore(state, auto_devour=False):
    ...
    if enc is None:
        finds_cave = [('You find a patch of hipokute grass glowing faintly.', ('hipokute_grass', 2)), ('A small magic crystal sits embedded in stone. You pry it free.', ('magic_crystal_shard', 1)), ('Nothing stirs. The silence feels heavy.', None), ('Water drips somewhere far off. You drink and feel slightly restored.', 'heal')]
        finds_forest = [('You fell a few trees and trim the timber for transport.', ('timber', 3)), ('You quarry loose stone blocks near the streambed.', ('stone', 2)), ('Hipokute flowers sway in the breeze. You gather an armful.', ('hipokute_grass', 2)), ('Nothing stirs. Birds circle lazily overhead.', None)]
...
async def village_explore(state):
    ...
    if not flags.get('village_met'):
        await first_meeting(state)
        return
    if not flags.get('village_founded'):
        await found_village_offer(state)
        return
...
async def first_meeting(state):
    h = state['hero']
    print()
    for l in ui.text_panel(' SMOKE ON THE WIND ', 'A thin column of smoke rises ahead. You find a cluster of crude huts - and carnage. Black wolves circle what remains of a goblin village. A wounded goblin elder props himself against a broken spear and stares at you without fear.', ui.Y):
        print(l)
    print()
    c = await ui.choose("The elder rasps: 'Help us... please.'", ['Fight the wolf pack', 'Turn away'])
    if c == 1:
        print(ui.DIM + 'You melt into the brush. The howling fades behind you.' + ui.RESET)
        await ui.pause()
        return
    state['world']['flags']['village_met'] = True
    out = await battle_flow(state, ['dire_wolf', 'dire_wolf'], 'Goblin Village Defense')
    if out.get('result') != 'win':
        return
    print()
    for l in ui.text_panel(' AFTERMATH ', 'The survivors emerge - perhaps a hundred goblins, gaunt and terrified. The old chief kneels before you.', ui.Y):
        print(l)
    print()
    ...
    await naming_sys.naming_ceremony(state, 'goblin', level=2, discount_pct=discount)
    state['world']['flags']['village_founded'] = True
    state['world'].setdefault('unlocked_zones', [])
    refresh_recruit_pool(state)
    h.gold += 100
    await ensure_kingdom(state)
    print()
...
async def found_village_offer(state):
    h = state['hero']
    print()
    print(f'{ui.BW}The goblin elder waits at the village center, watching you expectantly.{ui.RESET}')
    c = await ui.choose('Grant them a name?', ['Hold the naming ceremony', 'Not yet'])
    if c == 0:
        ok, cost = naming_sys.can_name(h, 'goblin')
        discount = 0.5 if not ok else 0.0
        await naming_sys.naming_ceremony(state, 'goblin', level=2, discount_pct=discount)
...
async def ensure_kingdom(state):
    if state.get('kingdom') is None:
        from src.systems.kingdom import Kingdom
        state['kingdom'] = Kingdom()
        ui.voice(['A dominion requires governance.', 'Confirmed. Realm administration systems established.', 'Access them via [Tempest Governance] while at the settlement.'])
        await ui.pause()

async def random_daily_event(state):
    k = state['kingdom']
    roll = random.random()
    hostile = [fid for fid, rel in k.relations.items() if rel <= -30]
    if hostile and k.pending_siege is None and (roll < 0.3):
...
async def siege_event(state, faction_id):
    k = state['kingdom']
    h = state['hero']
    ...
    print()
    for l in ui.text_panel(f' SIEGE OF TEMPEST ', f"{f['name']} marches on your settlement! City defenses ({k.city_defense()}) blunt their assault by {defense_cut}%.", ui.R):
        print(l)
    print()
    c = await ui.choose('Command the defense?', ['Take the field personally', 'Let the walls hold (auto-resolve)'])
    ...
    if win:
        ...
        ui.voice(['The walls are breached...', f'Lost: {dname}, treasury plundered {plunder}g, {loss} souls gone.'])
    await ui.pause()

async def tournament_event(state):
    h = state['hero']
    print()
    for l in ui.text_panel(' TEMPEST TOURNAMENT ', 'Fighters gather for a martial exhibition before roaring crowds. Three victories would make your legend ring across the forest.', ui.Y):
        print(l)
    c = await ui.choose('Enter the arena?', ['Enter', 'Decline'])
    if c != 0:
        return
    roster = ['dire_alpha', 'armorsaurus', 'orc_general']
    for i, mid in enumerate(roster, 1):
        out = await battle_flow(state, [mid], f'Tournament Round {i}')
        if out.get('result') != 'win':
...
async def rest(state):
    ...
    if k is None and founded:
        await ensure_kingdom(state)
        k = state['kingdom']
    arc = await events_sys.arc_check(state)
    if arc == 'orc_war':
        await events_sys.orc_war_event(state)
    elif arc == 'walpurgis':
        await events_sys.walpurgis_event(state)
    elif arc == 'empire_wave':
        await events_sys.empire_wave_event(state)
    if k:
        ...
        if not any((e[0] == 'siege' for e in events)):
            await random_daily_event(state)
        if k.has('colosseum') and random.random() < 0.2 and (state['world']['day'] % 3 == 0):
            await tournament_event(state)
    autosave(state)
...
async def recruit_menu(state):
    ...
    while True:
        ...
        for l in ui.panel(' WILLING SOULS ', lines, ui.M):
            print(l)
        ci = await ui.choose('Name which one?', [f"{mdata.MONSTERS[c['species']]['name']}" for c in pool], allow_cancel=True)
        if ci is None:
            return
        cp = pool[ci]
        result = await naming_sys.naming_ceremony(state, cp['species'], cp['level'])
        if result:
            pool.pop(ci)
...
async def travel(state):
    ...
    i = await ui.choose('Travel where?', opts, allow_cancel=True)
    if i is None:
        return
...
async def shop_menu(state):
    ...
    while True:
        ...
        i = await ui.choose('Trade:', opts, allow_cancel=True)
        if i is None:
            return
        if i == len(opts) - 2:
            gold = 0
        ...
        price = item['value']
        if h.gold < price:
            await ui.pause(ui.R + 'Not enough gold.' + ui.RESET)
            continue
        if item['kind'] == 'gear':
            h.gold -= price
...
def build_payload(state):
    persist_world(state)
    payload = {'player': state['hero'].to_dict(), 'world': dict(state['world']), 'roster': list(state.get('roster', [])), 'captures': list(state.get('captures', []))}
...
async def hub_loop(state):
    while True:
        ...
        ui.header(f"Day {state['world']['day']} | {loc}", f'{h.name} Lv{h.level} | EP {ep:,} [{rank_from_ep(ep)}]')
        ...
        if party:
            ...
            print(f'  Party: {pl}')
        print()
        actions = [('Explore', lambda: explore(state)), ('Explore x5', lambda: explore_repeat(state, 5)), ('Roster', lambda: naming_sys.roster_menu(state))]
        ...
        actions += [('Rest (full recovery)', lambda: rest(state)), ('Character Status', lambda: status_view(state)), ('Skills & Mastery', lambda: skills_menu(state)), ('Stomach / Materials', lambda: predator_sys.stomach_menu(state)), ('Merchant' if not zid == 'goblin_village' else 'Village Stores', lambda: shop_menu(state)), ('Save Game', lambda: save_and_quit(state)), ('Quit to Title', None)]
        labels = [a[0] for a in actions]
        c = await ui.choose('What will you do?', labels)
        if c is None or c >= len(actions) - 1:
            await save_and_quit(state)
            return
        await _run_action(actions[c][1])
