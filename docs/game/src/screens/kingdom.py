...
def overview_lines(state, k):
    ...
    h = state["hero"]
    lines = [
        f"Treasury {ui.GOLD_C}{k.treasury:,}g{ui.RESET}   Income ~{ui.GOLD_C}{k.daily_income()}g/day{ui.RESET}   "
        f"Tax {k.tax_rate}%   Food {int(k.food)} ({k.food_production():.0f}/day vs -{k.food_consumption():.0f})",
        f"Population {ui.BW}{k.total_pop}{ui.RESET}/{k.pop_cap()}   "
        + "   ".join(f"{r.title()}: {n}" for r, n in k.pop.items()),
        f"Happiness [{ui.bar(int(k.happiness), 100, 16, color=ui.HP_C, show_nums=False)}] {int(k.happiness)}/100"
        f"   City Defense {ui.C}{k.city_defense()}{ui.RESET}   Research {ui.BC}{int(k.rp)} RP{ui.RESET}",
    ]
    routes = ", ".join(bdata.TRADE_ROUTES[r]["name"] for r in k.routes) or "none"
    lines.append(f"Trade Routes: {routes}")
    under = [(bdata.BUILDINGS[b]["name"], s["days_left"]) for b, s in k.buildings.items()
             if not s["done"] and s["days_left"] > 0]
    if under:
...
def kingdom_menu(state):
    ...
    while True:
        ...
        if c == 0:
            districts_menu(state, k)
        elif c == 1:
            workforce_menu(state, k)
        elif c == 2:
            economy_menu(state, k)
        elif c == 3:
            diplomacy_menu(state, k)
        elif c == 4:
            crafting_menu(state, k)
        elif c == 5:
            academy_menu(state, k)
        elif c == 6:
            chronicle_menu(state, k)
        else:
            return


def districts_menu(state, k):
    hero = state["hero"]
    while True:
        ui.clear()
        ui.header("DISTRICTS & CONSTRUCTION",
                  f"Treasury {ui.GOLD_C}{k.treasury}g{ui.RESET}")
        by_district = {}
        ...
        for did in order:
            ...
            for bid in by_district[did]:
                ...
                st = k.buildings[bid]
                mark = f"{ui.BG}[BUILT]{ui.RESET}" if st["done"] else (
                    f"{ui.Y}[{st['days_left']}d left]{ui.RESET}" if st["days_left"] > 0 else "[--]")
        ...
        if not opts:
            ui.pause("Nothing available to build. Press Enter...")
            return
        i = ui.choose("Start construction?", opts, allow_cancel=True)
        if i is None:
            return
        bid = flat[i]
...
def workforce_menu(state, k):
    while True:
        ...
        for l in ui.panel(" LABOR ", lines, ui.B):
            print(l)
        c = ui.choose("Adjust:", [f"{JOB_NAMES[j]} +/-5%" for j in JOBS] + ["Done"], allow_cancel=False)
        if c == len(JOBS):
            return
        j = JOBS[c]
        d = ui.choose("Which way?", ["+5%", "-5%"], allow_cancel=True)
        if d is None:
            continue
        ...
        if d == 0:
            ...
            if k.jobs[j] >= 5 and k.jobs[other] <= 95:
                k.jobs[j] -= 5
                k.jobs[other] += 5


def economy_menu(state, k):
    while True:
        ui.clear()
        ui.header("ECONOMY & TAXATION", f"Treasury {ui.GOLD_C}{k.treasury:,}g{ui.RESET}")
        income = k.daily_income()
        labor = int(k.total_pop * (k.tax_rate / 100.0) * 0.30)
        trade = sum(bdata.TRADE_ROUTES[r]["income"] for r in k.routes)
        lines = [
            f"Projected daily income: {ui.GOLD_C}{income}g{ui.RESET}",
            f"  Commerce+Routes: {trade > 0 and trade or 0}g from routes; labor tax contributes {labor}g",
            "",
            f"Tax rate: {ui.BY}{k.tax_rate}%{ui.RESET}   Happiness equilibrium: {int(k.happiness_eq())}",
            f"Food: {int(k.food)} stored | production {k.food_production():.0f}/day | consumption {k.food_consumption():.0f}/day",
        ]
        for l in ui.panel(" LEDGER ", lines, ui.Y):
            print(l)
        c = ui.choose("Actions:", [
            f"Set tax rate (current {k.tax_rate}%)",
            "View trade routes",
            "Back",
        ])
        if c == 0:
            ti = ui.choose("Tax level:", ["0% (beloved)", "10% (fair)", "20% (heavy)", "30% (crushing)"], allow_cancel=True)
            if ti is not None:
                k.tax_idx = ti
        elif c == 1:
            rl = []
            for rid, r in bdata.TRADE_ROUTES.items():
                status = f"{ui.BG}[ACTIVE +{r['income']}g/d]{ui.RESET}" if rid in k.routes else \
                         f"{ui.DIM}[needs {r['req_rel']} rel with {bdata.FACTIONS[r['faction']]['name']}]{ui.RESET}"
...
def diplomacy_menu(state, k):
    ...
    while True:
        ...
        sel = ui.choose("Treat with which power?", [bdata.FACTIONS[f]["name"] for f in fids], allow_cancel=True)
        if sel is None:
            return
        fid = fids[sel]
        ...
        actions.append("Back")
        a = ui.choose(f"{f['name']}:", actions, allow_cancel=False)
        if a == 0:
...
def crafting_menu(state, k):
    ...
    while True:
        ...
        for l in ui.panel(" RECIPES ", lines, ui.Y):
            print(l)
        if not craftable:
            ui.pause("Nothing craftable yet. Enter...")
            return
        i = ui.choose("Craft what?", [idata.ITEMS[r['id']]['name'] for r, _ in craftable], allow_cancel=True)
        if i is None:
            return
...
def academy_menu(state, k):
    while True:
        ...
        for l in ui.panel(" DOCTRINES ", lines, ui.BC):
            print(l)
        if not avail:
            ui.pause("Nothing to learn right now. Enter...")
            return
        i = ui.choose("Research which doctrine?", [p["name"] for p in avail], allow_cancel=True)
        if i is None:
            return
        p = avail[i]
        k.rp -= p["rp"]
