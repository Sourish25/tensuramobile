...
JOB_NAMES = {"farm": "Farming", "smith": "Smithing", "guard": "Guard Duty",
             "builder": "Construction", "research": "Research", "service": "Commerce"}
...
class Kingdom:
    ...
    def daily_income(self):
        ...
        labor_tax = self.total_pop * (self.tax_rate / 100.0) * 0.30
        service_mult = 1 + (self.jobs["service"] / 100.0) * 0.6
        income_pct = 1.10 if self.has_perk("logistics") else 1.0
    ...
    def research_gain(self):
        ...
        return self.workforce() * (self.jobs["research"] / 100.0) * 0.40 * mult
    ...
    def daily_tick(self, hero, day):
        ...
        hipokute_bid = next((b for b in self.built_list()
                             if bdata.BUILDINGS[b].get("daily_material")), None)
        if hipokute_bid:
    ...
    def to_dict(self):
        return {
            ...
            "tax_idx": self.tax_idx,
            "pop": dict(self.pop),
            "buildings": {b: dict(s) for b, s in self.buildings.items()},
            "jobs": dict(self.jobs),
            "relations": dict(self.relations),
            "routes": list(self.routes),
            "rp": int(self.rp),
            "perks": list(self.perks),
            "pending_siege": dict(self.pending_siege) if self.pending_siege else None,
            "envoy_cd": dict(self.envoy_cd),
            "gift_cd": dict(self.gift_cd),
