...
RACE_AWAKEN_ULTIMATE = {'slime': ['raphael', 'belzebuth', 'soul_consume'], 'goblin': ['amaterasu_u', 'flame_breath_u'], 'lizardman': ['nyarlathotep', 'melt_slash'], 'ogre': ['susano_oh', 'creations_blade'], 'direwolf': ['hastur', 'time_stop']}
UNIQUE_TO_ULTIMATE = {'great_sage': 'raphael', 'predator': 'belzebuth'}
FESTIVAL_GRANTS = {'goblin': ['steel_thread', 'fear_roar', 'heal_minor'], 'dire_wolf': ['wind_cutter', 'lightning_bolt', 'gravity_flight'], 'orc_soldier': ['monstrous_str', 'rot_aura', 'body_slam'], 'orc_rider': ['monstrous_str', 'tempest_scale', 'sharp_horn'], 'horned_rabbit': ['sprint', 'paralysis_breath', 'sticky_steel_thread'], 'giant_ant': ['paralysis_breath', 'body_slam', 'stone_bullet'], 'black_spider': ['sticky_steel_thread', 'poison_breath', 'steel_thread'], 'cave_centipede': ['paralysis_breath', 'poison_breath', 'claw_swipe'], 'giant_bat': ['drain_touch', 'ultrasonic_wave', 'water_blade'], 'lizardman': ['water_blade', 'gravity_flight', 'flame_aura'], 'dire_alpha': ['wind_cutter', 'lightning_bolt', 'time_stop']}
...
async def check_awakening(state):
    h = state['hero']
    ...
    await ui.pause()
    ui.voice(['Warning. Unique conditions have been met.', f'Soul quota fulfilled: {h.souls:,} / {SOULS_FOR_AWAKENING:,}.', 'Initiating Harvest Festival.'])
    print()
    await ui.pause()
    h.demon_lord = True
    race_id = h.race_id
    ...
    h.hp = h.max_hp
    h.mp = h.max_mp
    stage_names = {'slime': 'Demon Slime', 'goblin': 'Oni King', 'lizardman': 'Dragon Lord-tier', 'ogre': 'Awakened Oni', 'direwolf': 'Divine Wolf'}
    print()
...
async def check_godhood(state):
    ...
    ui.clear()
    print()
    for l in ui.text_panel(" THE ADMINISTRATOR'S SEAT ", "Michael's authority dissolves into your hands like snow in starlight. Somewhere beyond the sky, a door no mortal built swings open.", ui.BY):
        print(l)
    print()
    await ui.pause()
    ui.voice(['Ultimate Dominion seized.', 'Administrator rights transferred to individual [' + h.name + '].', 'Detected: dormant authority [Azathoth]. Merge proposed.'])
    print()
    c = await ui.choose('Merge Raphael and Belzebuth into Azathoth, God of the Void?', ['Become.', 'Not yet'])
    if c != 0:
    ...
    if gz not in unlocked:
        unlocked.append(gz)
    print()
    for l in ui.text_panel(' GODHOOD ', 'You are no longer a monster, a demon lord, or even a dragon. You are the thing the World reaches toward when it dreams. And still - the grind goes on. Countless worlds wait behind the Gates.', ui.BY):
        print(l)
    print()
