...
async def naming_ceremony(state, species_key, level, source='capture', discount_pct=0.0):
    hero = state['hero']
    t = MONSTERS[species_key]
    ...
    lines = [f"Species: {t['name']}  (Lv{level})", f"Magicule cost: {ui.M}{cost}{ui.RESET} of {hero.stats['mp']} total magicules", f'Current usable: {hero.max_mp}', '', ui.DIM + "Naming burns your own life force into another's soul. It never fully returns." + ui.RESET]
    ...
    c = await ui.choose('Proceed?', opts, allow_cancel=True)
    if c is None:
        return None
    if c == 0:
        ...
        nm = ''
        while not nm.strip():
            nm = (await webbridge.ask_text('Name:') or '').strip()[:16]
            if not nm.strip():
    ...
    msgs = [f'Naming executed. Magicules transferred: {cost}.', f'Individual [{name}] has received the name of [{hero.name}].']
    if evolved:
    ...
    assign = await ui.choose(f'[{name}] awaits orders.', ['Join active party', 'Hold in reserve'], allow_cancel=True)
    if assign == 0:
        await set_party_slot(state, len(roster) - 1, join=True)
    await ui.pause()
    return sub
...
async def set_party_slot(state, index, join):
    subs = load_roster(state)
    if join:
    ...
    elif subs[index].assignment == 'party':
        subs[index].assignment = 'bench'
    save_roster(state, subs)
STANCES = ['aggressive', 'balanced', 'defensive', 'support']

async def roster_menu(state):
    while True:
        subs = load_roster(state)
        ui.clear()
        ui.header('ROSTER', f"{len(subs)} named | Party {sum((1 for s in subs if s.assignment == 'party'))}/3")
        if not subs:
            body = ['No one bears your name yet.', '', ui.DIM + 'Spare defeated monsters and offer them a name,' + ui.RESET, ui.DIM + 'or recruit willing souls at the goblin village.' + ui.RESET]
        ...
        for i, s in enumerate(subs):
            ...
            lines.append(f"{col}{i + 1}. {s.glyph} {s.name:<14}{ui.RESET} {form:<20} Lv{s.level:<3} ATK {s.stats['atk']:<4} MAG {s.stats['mag']:<4} AGI {s.stats['agi']:<4} {tag} {s.stance:<10} Loyal {s.loyalty}%")
        ...
        opts.append('Back')
        c = await ui.choose('Manage:', opts, allow_cancel=False)
        if c == len(opts) - 1 or not subs:
            return
        si = await ui.choose('Which follower?', [f'{s.name}' for s in subs], allow_cancel=True)
        if si is None:
            continue
        if c == 0:
            await set_party_slot(state, si, join=subs[si].assignment != 'party')
        elif c == 1:
            st_i = await ui.choose('Stance:', STANCES, allow_cancel=True)
            if st_i is not None:
                subs[si].stance = STANCES[st_i]
                save_roster(state, subs)
        elif c == 2:
            from src.core import webbridge
            nm = (await webbridge.ask_text('New name:') or '').strip()[:16]
            if nm:
                old = subs[si].name
                subs[si].name = nm
                save_roster(state, subs)
...
async def captures_menu(state):
    ...
    while True:
        ...
        if not caps:
            for l in ui.text_panel(' NONE ', ['You hold no spared monsters.', '', ui.DIM + "After a battle, choose 'Spare' instead of 'Devour'" + ui.RESET, ui.DIM + 'to bring a defeated foe back here for naming.' + ui.RESET]):
                print(l)
            await ui.pause()
            return
        lines = []
        ...
        for l in ui.panel(' AWAITING NAMES ', lines, ui.M):
            print(l)
        ci = await ui.choose('Name which one?', [f"{MONSTERS[c['species']]['name']}" for c in caps], allow_cancel=True)
        if ci is None:
            return
        cp = caps[ci]
        ...
        if not ok:
            ...
            await ui.pause()
            continue
        result = await naming_ceremony(state, cp['species'], cp['level'])
        if result:
            caps.pop(ci)
