...
async def offer_combination_menu(state):
    hero = state['hero']
    while True:
        ready = []
        known = set(hero.skills.keys())
        ...
        for a, b, child in ready:
            ...
            opts.append(f"{get_skill(a)['name']} ({hero.skills[a]['mastery']:.0f}%) + {get_skill(b)['name']} ({hero.skills[b]['mastery']:.0f}%) -> {combo_name}")
        i = await ui.choose('Synthesize which skill?', opts, allow_cancel=True)
        if i is None:
            return
        a, b, child = ready[i]
        hero.learn_skill(child, mastery=50)
        ui.voice([f'Detected compatible skill structures.', f"Combining [{get_skill(a)['name']}] and [{get_skill(b)['name']}].", f'Unique synthesis successful.', f"New extra skill [{get_skill(child)['name']}] acquired."])

async def stomach_menu(state):
    hero = state['hero']
    while True:
        ui.clear()
        ...
        c = await ui.choose('Stomach actions:', opts)
        if c == 0:
            gold = 0
