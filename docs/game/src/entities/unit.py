...
def rank_from_ep(ep):
    ...
    if ep >= 400:
        return 'E'
    return 'F'
RACE_STARTER_NUKE = {'slime': 'icicle_lance', 'goblin': 'fireball', 'lizardman': 'water_blade', 'ogre': 'stone_bullet', 'direwolf': 'wind_cutter'}

class Unit:

    def __init__(self, name, glyph='?', color='\x1b[37m'):
        self.name = name
        self.glyph = glyph
        self.color = color
        self.level = 1
...
class Player(Unit):
    ...
    def can_afford_name(self, target_ep):
        ...
        return (self.stats['mp'] - self.mp_scar - cost >= self.stats['mp'] * 0.1, cost)
    ...
    def to_dict(self):
        return {'kind': 'player', 'name': self.name, 'race_id': self.race_id, 'level': self.level, 'xp': self.xp, 'gold': self.gold, 'souls': self.souls, 'stats': dict(self.stats), 'skills': {sid: d['mastery'] for sid, d in self.skills.items()}, 'latent_unlocked': self.latent_unlocked, 'stomach': dict(self.stomach), 'materials': dict(self.materials), 'consumables': dict(self.consumables), 'gear': dict(self.gear), 'mimic_forms': list(self.mimic_forms), 'devour_count': self.devour_count, 'kills': self.kills, 'phase': self.phase, 'mp_scar': self.mp_scar, 'demon_lord': self.demon_lord, 'godhood': self.godhood, 'auto': self.auto, '_evo_stage': getattr(self, '_evo_stage', None)}
...
class Subordinate(Unit):
    ...
    def __init__(self, name, species_key, level=1):
        ...
        super().__init__(name, t['glyph'], '\x1b[92m')
    ...
    def to_dict(self):
        return {'kind': 'sub', 'name': self.name, 'species': self.species, 'level': self.level, 'xp': getattr(self, 'xp', 0), 'stats': dict(self.stats), 'skills': {sid: d['mastery'] for sid, d in self.skills.items()}, 'loyalty': self.loyalty, 'assignment': self.assignment, 'stance': self.stance, 'evolve_name': self.evolve_name, 'evolve_glyph': self.evolve_glyph}
