...
RELICS = [
    {"id": "genesis_relic_asura", "name": "Asura, Blade of Wrath", "min_tier": 2},
    {"id": "genesis_relic_ark", "name": "Ark, First Sword", "min_tier": 4},
    {"id": "genesis_relic_crown", "name": "Crown of Dawn", "min_tier": 6},
    {"id": "genesis_relic_mirror", "name": "Mirror of Origins", "min_tier": 8},
    {"id": "genesis_relic_spring", "name": "Eternal Spring", "min_tier": 10},
    {"id": "genesis_relic_compass", "name": "Star Compass", "min_tier": 12},
    {"id": "genesis_relic_grail", "name": "Twilight Grail", "min_tier": 14},
]

WORLD_PREFIX = ["The Rust", "The Silent", "The Burning", "The Drowned", "The Hollow",
                "The Gilded", "Weeping", "Howling", "Crystal", "Endless"]
WORLD_SUFFIX = ["Expanse", "Steppe", "Ocean", "Garden", "Citadel", "Wastes", "Archipelago",
                "Cathedral", "Forest", "Machine"]

MODIFIERS = [
    {"id": "hardy", "name": "Hardy Fauna", "desc": "Enemies +40% HP", "hp": 1.4},
    {"id": "feral", "name": "Feral Evolution", "desc": "Enemies +30% ATK/MAG", "atk": 1.3},
    {"id": "rich", "name": "Dense Magicules", "desc": "Souls x2", "souls": 2},
    {"id": "opulent", "name": "Opulent Ruins", "desc": "Essence x1.5", "essence": 1.5},
    {"id": "brittle", "name": "Brittle Existence", "desc": "Enemies -25% HP", "hp": 0.75},
    {"id": "swift", "name": "Accelerated Time", "desc": "Enemies +25% AGI", "agi": 1.25},
    {"id": "holy_land", "name": "Sanctified Land", "desc": "Angelic remnants (+seraphs)", "pool": "angel"},
    {"id": "swarm", "name": "Swarm Worlds", "desc": "Insectoid dominant (+insectors)", "pool": "swarm"},
]

ESSENCE_UPGRADES = [
    {"id": "void_blessing", "name": "Blessing of the Void", "base_cost": 20,
     "desc": "+5% to all your stats, permanently."},
    {"id": "legion_doctrine", "name": "Legion Doctrine", "base_cost": 25,
     "desc": "+5% to ALL named subordinates' stats, permanently."},
    {"id": "fate_weaving", "name": "Fate Weaving", "base_cost": 15,
     "desc": "+3% crit and dodge, permanently."},
]


def default_postgod():
    return {"gates_cleared": 0, "essence": 0, "relics": [], "authority": 0,
            "offers": None, "upgrade_counts": {}}
...
def make_offers(pg):
    ...
    for _ in range(3):
        ...
        offers.append({
            "name": f"{random.choice(WORLD_PREFIX)} {random.choice(WORLD_SUFFIX)}",
            "mods": mods,
            "tier": tier,
        })
    return offers
...
def relic_check(state, tier):
    ...
    for rel in RELICS:
        ...
        if tier >= rel["min_tier"]:
            ...
            ui.voice([
                f"GENESIS RELIC recovered: [{rel['name']}].",
                "Its authority weaves into your existence. All stats permanently increased.",
                f"Relics: {len(pg['relics'])}/7.",
            ])
            if len(pg["relics"]) >= 7:
...
def run_gate(state, offer):
    ...
    print()
    for l in ui.text_panel(f" DIMENSIONAL GATE T{tier}: {offer['name'].upper()} ",
                           " | ".join(m["desc"] for m in offer["mods"]), ui.BY):
        print(l)
    c = ui.choose("Step through?", ["Enter the world", "Withdraw"])
    if c != 0:
        return

    pools = {
        "default": ["cryptid_drone", "dracobeast"],
        "angel": ["seraph_minion", "cryptid_drone"],
        "swarm": ["cryptid_drone", "insector_warrior"],
    }
    ...
    print()
    ui.voice([
        f"World heart consumed. [{offer['name']}] folds back into potential.",
        f"+{gained:,} souls. +{essence} divine essence.",
    ])
...
def gates_menu(state):
    pg = ensure_postgod(state)
    while True:
        ui.clear()
        ui.header("CELESTIAL GATES", f"{pg['gates_cleared']} worlds devoured")
        lines = [
            f"Divine Essence: {ui.BY}{pg['essence']}{ui.RESET}   Relics: {ui.BM}{len(pg['relics'])}/7{ui.RESET}",
            f"Divine Authority stacks: {ui.GOLD_C}{pg['authority']}{ui.RESET}",
            "",
        ]
        ...
        c = ui.choose("Choose a world:", opts, allow_cancel=False)
        if c is None or c == len(opts) - 1:
            return
        if opts[c].startswith("T") and "  [" in opts[c]:
            run_gate(state, pg["offers"][c])
            pg["offers"] = None
            continue
        label = opts[c]
        ...
        if label == "Essence Works":
            essence_menu(state, pg)
            continue
        if label == "World Recreator":
            prestige_menu(state, pg)
            continue


def essence_menu(state, pg):
    while True:
        ui.clear()
        ...
        for l in ui.panel(" PERMANENT BLESSINGS ", lines, ui.BY):
            print(l)
        i = ui.choose("Purchase which blessing?", [a[0]["name"] for a in avail], allow_cancel=True)
        if i is None:
            return
        up, cost = avail[i]
        if pg["essence"] < cost:
...
def prestige_menu(state, pg):
    h = state["hero"]
    ui.clear()
    ui.header("WORLD RECREATOR", "Azathoth's deepest authority")
    lines = [
        f"Relics held: {len(pg['relics'])}/7   Authority stacks: {pg['authority']}   "
        f"Current multiplier: x{2 ** pg['authority']:,}",
        "",
        ui.DIM + "Unmake this world and seed it anew. Your legend, legion, relics," + ui.RESET,
        ui.DIM + "and divinity persist. The world forgets you - and begins again." + ui.RESET,
        "",
        f"Each rebirth: stats DOUBLED (+100%), full restoration,",
        f"+5000g starting treasury for the new realm.",
    ]
    ...
    if len(pg["relics"]) < 3:
        ...
        ui.pause()
        return
    c = ui.choose("Unmake creation?", [
        "Recreate the world (+1 Authority: ALL STATS x2)",
        "Not yet",
    ])
    if c != 0:
        return
    ...
    w["floor"] = 1
    w["flags"] = {"awakened": True, "godhood": True, "michael_defeated": True,
                  "walpurgis_done": True, "octagram_done": True, "empire_war_done": True,
                  "orc_war_done": True}
    w["unlocked_zones"] = ["sealed_cave", "jura_plains", "goblin_village", "celestial_gates"]
    w["bosses_slain"] = []
    w["recruit_pool"] = None
    ui.voice([
        f"Authority stack {pg['authority']} inscribed.",
        "A new world opens its eyes beneath you.",
        "The grind, eternal, continues.",
    ])
    ui.pause()
