...
def arc_check(state):
    ...
    if w["phase"] == 1 and k is not None and not flags.get("orc_war_done"):
        if not flags.get("orc_war_warned") and w["day"] >= 3:
            ...
            print()
            ui.voice([
                "Warning. A massive migration of starving orcs detected.",
                "Estimated arrival: two days.",
                "The horde is led by a Disaster-class existence.",
            ])
    ...
    if flags.get("octagram_done") and not flags.get("empire_war_done"):
        if not flags.get("empire_warned"):
            ...
            print()
            ui.voice([
                "Eastern Empire mobilized: 700,000 troops, 2,000 magitanks.",
                "Their target: your labyrinth. Their goal: annihilation.",
                "Defense waves will strike on consecutive days.",
            ])
            ui.pause()
        else:
            flags["empire_next"] -= 1
...
def orc_war_event(state):
    h = state["hero"]
    flags = state["world"]["flags"]
    k = state["kingdom"]
    print()
    for l in ui.text_panel(" THE ORC DISASTER ",
                           "The forest itself flees before them - two hundred thousand starving orcs "
                           "and their lord, a mountain of hunger wearing a crown of tusks.", ui.R):
        print(l)
    print()
    waves = [
    ...
    print()
    ui.voice([
        "The Orc Disaster has fallen.",
        f"Confirmed. Souls harvested: {h.souls:,}.",
        "Regional threat level: reduced. Your name spreads beyond the forest.",
    ])
...
def walpurgis_event(state):
    flags = state["world"].setdefault("flags", {})
    h = state["hero"]
    print()
    for l in ui.text_panel(" WALPURGIS ",
                           "A ring of black fire drags you into an otherworldly hall. Eight thrones. "
                           "Seven monsters old enough to remember the world's birth - and one seat, "
                           "new, waiting for you.", ui.BM):
        print(l)
    print()
    ...
    print()
    c = ui.choose("Claim your seat?", [
        "Accept. I am Rimuru's equal in nothing and my own self entirely - I claim the eighth seat.",
        "Stay silent",
    ])
    ...
    ui.voice([
        "The Octagram is complete.",
        "Demon Lord [" + h.name + "] recognized by the council.",
        "Rival trials unlocked: prove yourself to Milim, Leon, and Guy.",
    ])
    k = state.get("kingdom")
    if k:
...
TRIALS = [
    {"id": "charybdis_hunt", "name": "Hunt: Charybdis", "boss": "charybdis",
     "req_phase": 2, "repeatable": True,
     "desc": "A calamity-class spirit rampages near the lake. Massive souls await."},
    {"id": "remnant_hunt", "name": "Mop-Up: Orc Remnants", "boss": None,
     "squad": ["orc_soldier", "orc_rider", "orc_soldier"],
     "req_phase": 2, "repeatable": True,
     "desc": "Stragglers of the broken horde still pillage the plains. Harvest their souls."},
    {"id": "milim", "name": "Rival Trial: Milim Nava", "boss": "milim_trial",
     "req_phase": 4, "req_flag": "walpurgis_done",
     "desc": "The Destroyer wants her 'fun'. Recommended: post-awakening, ultimate skills."},
    {"id": "leon", "name": "Rival Trial: Leon Cromwell", "boss": "leon_trial",
     "req_phase": 4, "req_flag": "milim_defeated",
     "desc": "The Platinum Devil answers Milim's defeat with interest."},
    {"id": "guy", "name": "Rival Trial: Guy Crimson", "boss": "guy_trial",
     "req_phase": 4, "req_flag": "leon_defeated",
     "desc": "The oldest demon lord closes the circle. Survive his scrutiny."},
    {"id": "kondo", "name": "War Duel: Lt. General Kondo", "boss": "kondo_guardian",
     "req_phase": 5, "req_flag": "empire_war_done",
     "desc": "The Imperial Guardians' leader demands satisfaction."},
    {"id": "michael", "name": "THE ADMINISTRATOR: Michael", "boss": "michael_admin",
     "req_phase": 6, "req_flag": "kondo_defeated",
     "ultimate": True,
     "desc": "Only ultimates can pierce his aura. Seize Ultimate Dominion. Become."},
]


def trials_menu(state):
    flags = state["world"].setdefault("flags", {})
    while True:
        ui.clear()
        ...
        if not opts:
            ui.pause("No trials currently available. Enter...")
            return
        i = ui.choose("Accept which trial?", opts, allow_cancel=True)
        if i is None:
            return
        run_trial(state, avail[i])
        return


def run_trial(state, trial):
    h = state["hero"]
    from src.entities.unit import enemy_from_template
    ...
    c = ui.choose("Begin?", ["Fight!", "Withdraw"])
    if c != 0:
        return
    allies = naming_sys.party_subs(state)
...
def empire_wave_event(state):
    ...
    squads = {
        1: ["imperial_guard", "mercenary", "mercenary"],
        2: ["imperial_guard", "imperial_guard"],
        3: ["kondo_guardian"],
    }
    squad = squads.get(wave_no, ["imperial_guard", "imperial_guard"])
    print()
    for l in ui.text_panel(f" EMPIRE INVASION - WAVE {wave_no}",
                           "Magitanks crest the ridge. The Empire's vanguard tests your labyrinth's teeth.", ui.R):
        print(l)
    from src.entities.unit import enemy_from_template
    enemies = [enemy_from_template(mid, 1.0 + wave_no * 0.12) for mid in squad]
    c = ui.choose("Response:", ["Sortie personally", "Labyrinth defenses handle it"])
